...
class Todo(tk.Tk):
    ...
    def task_is_exist(self, task=None):
        task_query = 'SELECT task FROM tasks WHERE task=?'
        task_data = [task]
        result_query = self.runQuery(task_query, task_data, receive=1)
        if result_query:
            return result_query
        else:
            return None
    ...
    def add_task(self, event=None, task_text=None, from_db=False):
        if not task_text:
            ...
            if self.task_is_exist(task_text):
                msg.showwarning(task_text + ' is Exist', 
                    task_text + ' is Exist, please try again.')
